...
namespace tree_m {
	...
	class BTreeNode
	{
		...
		string *sequence;//the leaf node point to raw sequence
	...
	public:
		...
		//get key value of current node
		int getkey(BTreeNode *y);

		//get sequence of current leaf
	...
	void BTree::insert(uint64_t k,string line)
	{
		...
		else // If tree is not empty 
		{
			...
			if (root->n == 2 * t - 1)
			{
				...
				if (s->keys[0] < k)
					i++;
				s->C[i]->insertNonFull(k,new string(line));

				// Change root 
				root = s;
			}
			else  // If root is not full, call insertNonFull for root 
				root->insertNonFull(k,new string(line));
		}
	}
	...
	//get key value of current node
	int BTreeNode::getkey(BTreeNode *node)
	{
		return *keys;
	}

	//get sequence of current leaf
	std::string BTreeNode::getsequence(BTreeNode *node)
	{
		return *sequence;
	}

	//get key value of current node
	int BTree::getkey(BTreeNode *node)
	{
		return getkey(node);
	}

	//get sequence of current leaf
	std::string BTree::getsequence(BTreeNode *node)
	{
		return getsequence(node);
	}
}